import { Client, GatewayIntentBits, Partials, REST, Routes, SlashCommandBuilder, PermissionFlagsBits, ActionRowBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder } from 'discord.js';
export class DiscordService {
    ...
    token;
    client;
    userPromptCache = new Map(); // userId -> prompt
    constructor(configService, memoryService, aiService, knowledgeService, token) {
    ...
    async registerCommands() {
        ...
        const commands = [
            new SlashCommandBuilder()
                ...
                .addSubcommand(sub => sub.setName('set-instructions')
                ...
                .addSubcommand(sub => sub.setName('allow-channel')
                ...
                .addSubcommand(sub => sub.setName('remove-channel')
    ...
    async handleInteraction(interaction) {
        ...
        if (interaction.commandName === 'bot-setup') {
            ...
            try {
                ...
                const options = files.slice(0, 25).map(file => new StringSelectMenuOptionBuilder()
            ...
            try {
                ...
                // Fetch context
                let recentMessages = [];
                if (interaction.channel && 'messages' in interaction.channel && typeof interaction.channel.messages.fetch === 'function') {
                    try {
                ...
                // Update memory
                const messagesToSummarize = [...sortedMessages, { role: 'assistant', content: responseText }];
    ...
    async handleMessage(message) {
        ...
        try {
            ...
            if ('messages' in message.channel && typeof message.channel.messages.fetch === 'function') {
    ...
    async sendChunkedReply(target, content) {
